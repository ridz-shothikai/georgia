...
export const createUser = async (req: Request, res: Response): Promise<void> => {
  try {
    const { email, password, role, assignedApps } = req.body;
    ...
    // Validate role
    if (!["user", "gadhs-it-staff", "director", "manager", "clerk", "admin", "superadmin"].includes(role)) {
      res.status(400).json({
        success: false,
 ...
 */
export const bulkUpdateUserApps = async (req: Request, res: Response): Promise<void> => {
  try {
